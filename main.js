...

{
  ...
  const gerneElement = document.getElementById('gerne');
  ...
  */
   const makeQuiz = (quiz) => {
    ...
    gerneElement.textContent = `【ジャンル】 ${quiz.category}`;
  ...
  */
  ...
  const shuffle = (array) => {
    const copidArray = array.slice();

    for (let i = copidArray.length - 1; i >= 0; i--) {
      const rand = Math.floor(Math.random() * (i + 1));
      [copidArray[i], copidArray[rand]] = [copidArray[rand], copidArray[i]]
    }
    return copidArray;
  };
